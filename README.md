...
4. 用户检查内容、格式、隐私和版权风险。
5. 用户明确要求发布后，再移动到对应的 `files/课程目录/`。
6. 使用 `scripts/publish_review_file.py` 发布文件，或手动移动文件后使用 `scripts/add_file_to_index.py` 将文件信息写入 `data/files.json`。
7. 在 `tasks/task-log.md` 记录任务。
8. 运行 `scripts/pre_publish_check.py` 和 `scripts/validate_files.py` 自检。
...
请先保存成品到 review/[课程目录]/ 等我检查。
我确认发布后，再移动到 files/[课程目录]/，更新 data/files.json 和 tasks/task-log.md，并运行发布前检查。
不要上传原始资料。
```
